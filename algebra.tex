...
\section{Structural Operations: A General Framework}
...
\subsubsection{Clock and Megaphone Points}
\label{subsubsec:clock-megaphone}
...
prenex, we realize such phase boundaries as ordinary binders owned by a
...
\paragraph{Megaphone point.}
Given a $K$-player formula
\[
  \Phi \;=\; P_{i_1} x_1.\,\dots.\,P_{i_n} x_n.\,\langle \varphi_0,\dots,\varphi_{K-1}\rangle,
\]
its \emph{megaphoned} version is the $(K{+}1)$-player formula
\[
  \mathsf{Meg}(\Phi)
  \;:=\;
  \Phi \cdot P_{\mathsf{Meg}}\,x_1.\;\dots.\;P_{\mathsf{Meg}}\,x_n.\;\langle \varphi_0,\dots,\varphi_{K-1}\rangle
\]
where $\mathsf{Meg}$ is a fresh role and each $u_{x_k}$ is fresh. Since the
megaphone binders are placed \emph{after} $\Phi$, they see all $x_k$; we assume
a fixed echo strategy $\sigma_{\mathsf{Meg}}(u_{x_k})=x_k$ (the role is
non-strategic). This adds no new objectives, only copies of earlier values.

\paragraph{Megaphone sequencing.}
Given two formulas $\Phi$ and $\Psi$ over the same player set
$\{0,\dots,K{-}1\}$, define their \emph{megaphone sequencing}, written
\[
  \Phi \;!\; \Psi,
\]
by inserting per-variable echoes of $\Phi$ at the cut and making $\Psi$ see
them:
\[
  \Phi \;!\; \Psi
  \;:=\;
  \Phi \cdot
    P_{\mathsf{Meg}}\,x_1.\;\dots.\;P_{\mathsf{Meg}}\,x_m.\Psi,
\]
where $\{x_1,\dots,x_m\}$ are the variables bound in the left part $\Phi$.
Intuitively, the megaphone \emph{re-utters} $x$ and is made visible to all
binders of $\Psi$, so later moves can condition on the previously hidden $x$'s.

\paragraph{Informal view.}
In the visibility graph, a clock introduces a single node that every
later node must see, but that carries no data. A megaphone introduces a
...
(nodes labeled by $(\mathrm{name},\mathrm{player},\mathrm{dom})$).
A \emph{splice instance} is a finite labeled DAG $I=(V_I,E_I)$ with injective
...
from $\Phi$ and $\Psi$. Its prenex script is any topological linearization of
